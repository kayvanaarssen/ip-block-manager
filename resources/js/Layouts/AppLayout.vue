...
const appName = computed(() => appSettings.value?.app_name || 'IPBlock')
            ...
            <div class="flex items-center h-16 px-6 border-b border-gray-200 dark:border-gray-800">
                <Link :href="route('dashboard')" class="flex items-center gap-2.5 min-w-0">
                    ...
                    <template v-if="appSettings?.logo_light || appSettings?.logo_dark">
                        ...
                        <img v-if="appSettings.logo_dark" :src="appSettings.logo_dark" :alt="appName" class="w-8 h-8 object-contain hidden dark:block" />
                        <img v-if="appSettings.logo_light && !appSettings.logo_dark" :src="appSettings.logo_light" :alt="appName" class="w-8 h-8 object-contain hidden dark:block" />
                        <img v-if="!appSettings.logo_light && appSettings.logo_dark" :src="appSettings.logo_dark" :alt="appName" class="w-8 h-8 object-contain dark:hidden" />
                    </template>
                    <!-- Default logo -->
                    <div v-else class="w-8 h-8 bg-primary-600 rounded-lg flex items-center justify-center shrink-0">
            ...
            <nav class="p-4 space-y-1 flex-1">
                ...
                <div class="pt-4 mt-4 border-t border-gray-200 dark:border-gray-800">
                    ...
                    <Link :href="route('settings.edit')"
                        :class="[page.url.startsWith('/settings') ? 'bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800']"
                        class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors"
                        @click="showSidebar = false">
